"""Backtest: SL/TP model on 2025 only."""
...
for y in [2025]:
    ...
    for i in range(60, len(m5) - 15):
        ...
        # SL/TP model filter
        if expected == "BUY" and buy_probs[i] < 0.60:
            continue
        if expected == "SELL" and sell_probs[i] < 0.60:
            continue
        ...
        for j in range(i + 1, min(i + 48, len(m5) - 1)):
            ...
            if expected == "BUY":
                ...
                if fh >= tp: bal += prof; wins += 1; break
            else:
                if fh >= sl: bal += prof; break
                if fl <= tp: bal += prof; wins += 1; break
        else:
    ...
    print(f"{y}: {total:>4} tr WR={wr:>5.1f}% PF={pf:>5.2f} PnL=${bal-20:>+8.2f} DD=${dd:>8.2f} Bal=${bal:>8.2f} [{elapsed:.0f}s]")
