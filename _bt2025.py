...
m1 = client.download_range(2024, 2025)
from_dt = datetime(2025, 1, 1); to_dt = datetime(2025, 12, 31, 23, 59)
...
N = len(m5)
tp = time.time()
print(f"M5: {N} H1: {len(h1)} [{tp-t0:.0f}s]", flush=True)
...
if not (i % 50000): print(f"  atr {i}/{N}", flush=True)
...
print(f"signals={sig_count} [{time.time()-tp:.0f}s]", flush=True)
tp = time.time()

meta = MetaStrategy(); scaler = EquityScaler(); scaler.initialize(INITIAL_BALANCE); bal = INITIAL_BALANCE; trds = []; cur = None; cl = 0; cd = None
