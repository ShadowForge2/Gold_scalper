...
if _HAS_TORCH:
    ...
    class CandleBrainTransformer(nn.Module):
        def __init__(
            self,
        ...
        def forward(
            ...
            entry_logits = self.entry_head(last)
            confidence = self.conf_head(last)
            mgmt_logits = self.mgmt_head(last)

            return entry_logits, confidence, mgmt_logits


class CandleBrain:
    """Production wrapper — loads model, runs inference, manages state."""

    def __init__(self, model_path: str = "models/candle_brain.pt", device: str = "cpu"):
        self.model = None
        self.device = device
        ...
        try:
            self.model = CandleBrainTransformer()
    ...
    def predict(
        ...
        if features.shape != (SEQ_LEN, N_FEATURES):
            if features.shape[1] != N_FEATURES:
                return None, 0.0, None
            if features.shape[0] > SEQ_LEN:
                features = features[-SEQ_LEN:]
            elif features.shape[0] < SEQ_LEN:
                pad = np.zeros((SEQ_LEN - features.shape[0], N_FEATURES), dtype=np.float32)
        ...
        with torch.no_grad():
            entry_logits, confidence, mgmt_logits = self.model(tensor)

        entry_probs = F.softmax(entry_logits, dim=-1).squeeze(0).cpu().numpy()
        conf = confidence.item()
        mgmt_probs = F.softmax(mgmt_logits, dim=-1).squeeze(0).cpu().numpy()

        entry_idx = int(np.argmax(entry_probs))
        entry_action = ENTRY_ACTIONS.get(entry_idx, "NONE")
        entry_conf = float(entry_probs[entry_idx])

        mgmt_idx = int(np.argmax(mgmt_probs))
        mgmt_action = MGMT_ACTIONS.get(mgmt_idx, "HOLD")
        mgmt_conf = float(mgmt_probs[mgmt_idx])

        if entry_action == "NONE" or entry_conf < 0.5:
            return None, conf, mgmt_action
        if mgmt_action == "CLOSE" and mgmt_conf > 0.6:
            return None, conf, "CLOSE"

        return entry_action, conf, mgmt_action

    def should_enter(self, features: np.ndarray, min_confidence: float = 0.6) -> Tuple[bool, str, float]:
        action, conf, _ = self.predict(features)
        if action is None:
            return False, "NONE", conf
        if conf < min_confidence:
            return False, action, conf
        return True, action, conf

    def should_exit(self, features: np.ndarray, min_mgmt_conf: float = 0.6) -> Tuple[bool, float]:
        _, conf, mgmt = self.predict(features)
        if mgmt == "CLOSE" and conf >= min_mgmt_conf:
            return True, conf
        return False, conf


class CandleBrainPredictor:
    """Production wrapper — takes raw M1 DataFrames, returns decisions.

    predict(m1_df) → dict:
      {"action": "BUY"|"SELL"|"NONE",
       "confidence": float,
       "mgmt_action": "HOLD"|"CLOSE",
       "mgmt_confidence": float}
    """
    ...
    def predict(self, m1: pd.DataFrame) -> Optional[Dict]:
        if not self.ready or self.model is None:
            return None
        features = build_features_from_m1(m1)
        ...
        return {
            ...
            "raw_conf": conf,
            "expectancy": float(conf),
            "mgmt_action": mgmt_action,
            "mgmt_confidence": mgmt_conf,
        }
